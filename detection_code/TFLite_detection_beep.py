...
while(cap.isOpened()):
    ...
    if (x == 5):
        for i in range(2):
            if  scores[i] > 0.6: #or (classes[i] == 1.) or (classes[i] == 2.)):
                if ((classes[i] == 3.) or (classes[i] == 1.)):
                    t= np.linspace(0, 1, 1*44100, False)
                    note = np.sin(500 * t * 2 * np.pi)
                    
                    audio = note * (2**10 - 1) / np.max(np.abs(note))
                    audio = audio.astype(np.int16)
                    play_obl = sa.play_buffer(audio, 1, 4, 44100)
        x = 0
    x+=1            
